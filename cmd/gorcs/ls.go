...
type Ls struct {
	*State
	...
	CommandAction func(c *Ls) error
}

type UsageDataLs struct {
	*Ls
	Recursive bool
}

func (c *Ls) Usage() {
	err := executeUsage(os.Stderr, "ls_usage.txt", UsageDataLs{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Ls) UsageRecursive() {
	err := executeUsage(os.Stderr, "ls_usage.txt", UsageDataLs{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func (c *State) NewLs() *Ls {
	...
	v.CommandAction = func(c *Ls) error {

		err := cli.StateLs(c.files...)
		if err != nil {
