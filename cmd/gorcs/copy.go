...
type Copy struct {
	*AccessList
	...
	CommandAction func(c *Copy) error
}

type UsageDataCopy struct {
	*Copy
	Recursive bool
}

func (c *Copy) Usage() {
	err := executeUsage(os.Stderr, "copy_usage.txt", UsageDataCopy{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Copy) UsageRecursive() {
	err := executeUsage(os.Stderr, "copy_usage.txt", UsageDataCopy{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func (c *AccessList) NewCopy() *Copy {
	...
	v.CommandAction = func(c *Copy) error {
		...
		err := cli.AccessListCopy(c.fromFile, c.toFiles...)
		if err != nil {
